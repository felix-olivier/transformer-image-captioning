...
from src.tell.yolov3.utils.google_utils import *
from src.tell.yolov3.utils.parse_config import *
from src.tell.yolov3.utils.utils import *
...
def create_modules(module_defs, img_size, arc):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            try:
                # class probability  ->  sigmoid(p) = 1/nc
                p = math.log(1 / (modules.nc - 0.99))
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                ...
                if verbose:
                    ...
                    s = [list(x.shape)] + [list(out[i].shape)
                                           for i in module.layers]  # shapes
                    str = ' >> ' + \
                        ' + '.join(['layer %g %s' % x for x in zip(l, s)])
            ...
            if verbose:
                print('%g/%g %s -' %
                      (i, len(self.module_list), mtype), list(x.shape), str)
                str = ''
